...
## LOCAL TESTING VERSION -- 1 run, sequential (no doParallel)
...
methods <- c("kmeans")   # LOCAL TEST: was c("kmeans", "med_ney", "mclust")

n_iter <- 1   # LOCAL TEST: was 40
...
# LOCAL TEST: only the smallest (4-variable) combo is kept.
var_combos <- list(
  tb5 = list(
...
cat("Total tasks:", nrow(task_grid), "\n")   # should print 1 for this local-test config

## ------------------------------------------------------------------
## Sequential processing -- no doParallel / foreach / cluster
## ------------------------------------------------------------------

results <- vector("list", nrow(task_grid))

for (t in 1:nrow(task_grid)) {
  ...
  iter_file <- paste0(
    ...
    "_iter", iter, "_LOCALTEST_results.RData"
  )
  ...
  set.seed(iter)
  # Same GA hyperparameters as production: iter = 2000, pops = 50
  ...
  # Save immediately after this task
  #save(iter_result, file = iter_file)
