...
for (ssize in sample_sizes) {
  ...
  for (method in methods) {
    ...
    load(paste0("CV/", method, "d_lognorm_low_results_", ssize, "_cv.RData"))
    ...
    filename <- paste0("OUTPUT/ballbar_", datasets[1], "_results_", ssize, "_", method, ".Rdata")
    save(store, file = filename)

    cat("Finished sample size:", ssize, "method:", method, "-> saved to", filename, "\n")
