...
# One entry per dataset: where to load it from, the object name it
# loads as, the sample size, and the number of strata to target.
dataset_specs <- list(
  ecd = list(
    path       = "multivar_datasets/ecd_census_multivariate.RData",
    obj        = "df",
    ssize      = 3000,
    num_strata = 6
  )
...
for (dsname in names(dataset_specs)) {
  ...
  dfs_all[[dsname]] <- convert_labelled(local_env[[spec$obj]])
}
