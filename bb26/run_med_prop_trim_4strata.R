...
for (i in 1:length(type)) {
  ...
  for (iter in 1:40) {
    ...
    ## First clustering, no trim
    start.time <- Sys.time()
    result <- pam(data, k=num_strata, metric=type[i], nstart=25)
    
    ## Find outliers from first clustering
    dmat <- as.matrix(daisy(data, metric=type[i]))
    
    medoid_indices <- result$id.med
    own_medoid_idx <- medoid_indices[result$clustering]
    
    if (any(is.na(own_medoid_idx))) {
      stop("own_medoid_idx contains NA — check result$id.med and result$clustering")
    }
    n <- nrow(data)
    if (length(own_medoid_idx) != n) stop("length mismatch between data and own_medoid_idx")
    
    
    #own_medoid <- result$medoids[result$clustering]
    
    
    dist_to_medoid <- dmat[cbind(seq_len(n), own_medoid_idx)]
    
    str(result$id.med)
    str(own_medoid_idx)
    summary(dist_to_medoid)
    
    
    ## Define threshold for trimming
    trim_prop <- 0.05
    trim_threshold <- quantile(dist_to_medoid, probs = 1 - trim_prop)
    
    ## Trim accordingly
    trimmed_idx <- which(dist_to_medoid <= trim_threshold)
    outlier_idx <- which(dist_to_medoid > trim_threshold)
    
    
    ## Re-run clustering on trimmed data
    result_trim <- pam(data[trimmed_idx, ], k = num_strata, nstart=25)
    end.time <- Sys.time()
    time <- difftime(end.time, start.time, units="mins")
    
    ## Combine trimmed and add back non-trimmed as an additional strata
    df1 <- data.frame(data[trimmed_idx, ], result_trim$clustering)
    names(df1) <- c(vars, "strata")
    df2 <- data.frame(data[outlier_idx, ])
    df2$strata <- num_strata +1 
    names(df2) <- c(vars, "strata")
  
    df <- rbind(df1, df2)
    strata <- df$strata
    df_ns$strata <- strata 

    n <- calculate_sample_size(df=df, strata=df$strata, vars=vars, method='prop', ssize=ssize)
    print(sum(n))
  ...
  list(
    n=n, cv=cv, trace=trace, det=det, varcov=varcov, pca=pca,
    strata=strata, objective=type[i], time=time
  )
...
filename = paste0("OUTPUT/med_prop_trim", datasets[dta], "_results_4strata.Rdata")
