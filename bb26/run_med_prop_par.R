...
for (i in 1:length(type)) {
  ...
  list(
    ...
    strata=strata, objective=type[i]
  )
  ...
  store_det_all[[ type[i] ]]    <- do.call(rbind, lapply(results, `[[`, "det"))
  store_varcov_all[[ type[i] ]] <- do.call(rbind, lapply(results, `[[`, "varcov"))
  store_pca_all[[ type[i] ]]    <- do.call(rbind, lapply(results, `[[`, "pca"))
...
filename = paste0("OUTPUT/med_prop", datasets[dta], "_results.Rdata")
