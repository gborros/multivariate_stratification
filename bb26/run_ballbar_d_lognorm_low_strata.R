## Run File - loops over sample sizes and methods automatically
...
## ---- sample sizes and methods to loop over ----
n_strata <- c(4,6)
n_strata_name <- c("4strata", "6strata")

methods <- c("kmeans_", "med_ney", "mclust_")

for (i in seq_along(n_strata)) {

  num_strata   <- n_strata[i]        # actual strata count, e.g. 4 or 6
  strata_label <- n_strata_name[i]   # matching label, e.g. "4strata"/"6strata"
  ...
  for (method in methods) {
    ...
    load(paste0("CV/", method, "d_lognorm_low_results_", strata_label, "_cv.RData"))
    ...
    datasets <- c("d_lognorm_low")
    dfs <- list(d_lognorm_low)
    ssize <- 500
    sample <- c(ssize)
    cv_list <- list(df_cv)

    for (dta in 1:length(datasets)) {
      ssize_local <- sample[1]
      vars <- character()
      cfs <- character()
      sf <- dfs[[dta]]
    ...
    # dynamic output filename per sample size and method
    filename <- paste0("OUTPUT/ballbar_", datasets[1], "_results_", strata_label, "_", method, ".Rdata")
    save(store, file = filename)

    cat("Finished strata:", num_strata, "method:", method, "-> saved to", filename, "\n")
